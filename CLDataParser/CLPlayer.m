...
-(void)setColorsWithArray:(NSArray *)colors
{
    ...
    for ( NSUInteger i = 0; i < mutableColors.count; i++ )
    {
        ...
        NSInteger colorIndex = [[colors objectAtIndex:i] integerValue];
        
        [mutableColors replaceObjectAtIndex:i 
                                 withObject:[Constants colorFromAllColorsAtIndex:(NSUInteger) colorIndex]];
    }
...
-(NSImage *)colorizeImage:(NSImage *)image
{
    ...
    NSBitmapImageRep *imageRep = [[NSBitmapImageRep alloc] initWithCGImage:[image CGImageForProposedRect:nil context:nil hints:nil]];
    ...
    for ( int i = 0; i < imageRep.pixelsWide; i++ )
    {
        ...
        for ( int j = 0; j < imageRep.pixelsHigh; j++ )
        {
            ...
            NSString *colorName = [Constants nameForColorFromPalette:pixelColor];
            
            //Replace color from palette with color designed for location by player's own colors
            [imageRep setColor:[_colors objectForKey:colorName] atX:i y:j];
...
-(BOOL)isEqual:(id)object
{
    ...
    CLPlayer *otherPlayer = (CLPlayer *) object;
    
    if ( [otherPlayer.name isEqual:_name] ) { return YES; }
